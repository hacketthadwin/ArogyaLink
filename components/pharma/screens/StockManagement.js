...
export default function StockManagement() {
  ...
  const handleUploadReport = async () => {
    ...
    if (!result.canceled) {
      Alert.alert('Report Uploaded', 'Your pharmacy report has been successfully uploaded!');
    }
  };
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background || COLORS.white }]}>
      <View style={[styles.header, { paddingTop: insets.top + 20 }]}>
        <Text style={[styles.title, { color: theme.text || COLORS.black }]}>Stock Management</Text>
